...
func resolveExternalGoPluginFunc(logger Logger, interpreter *interp.Interpreter, funcName string) *reflect.Value {
	r, err := interpreter.Eval(funcName)
	if err != nil {
		logger.Error(err).Msg("failed to eval " + funcName)
		return nil
	}
	if IsPrimitiveReflectValue(r) {
...
func (me ExternalGoPluginContext) Init(logger Logger, fs afero.Fs, codeFile string) {
